...
class Word2VecParam extends Serializable {
  ...
  var maxLength: Int = -1
  var minCount = 0
  val tableSize = 1e8.toInt
  var ns_exponent = 0.75
  ...
  /**
    * Set ns_exponent. Used to shape the negative sampling distribution (default: 0.75).
    */
  def setNSExponent(ns_exponent: Double): this.type = {
    this.ns_exponent = ns_exponent
    this
  }
